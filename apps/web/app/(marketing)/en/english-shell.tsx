...
function Navbar() {
  ...
  return (
    <>
      <nav aria-label="Main navigation" className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrollY > 40 ? "bg-white/80 backdrop-blur-xl shadow-sm border-b border-[#F5F5F7]" : "bg-white/50 backdrop-blur-sm"}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            ...
            <Link href="/" className="text-[13px] text-[#6E6E73] hover:text-[#1D1D1F] transition-colors font-medium flex items-center gap-1.5" title="Deutsch">
              <Globe size={14} /> DE
            </Link>
          ...
          <div className="flex md:hidden items-center gap-3">
            ...
            <button onClick={() => setMenuOpen(!menuOpen)} className="text-[#6E6E73] p-1" aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen}>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 bg-white pt-14 px-6 md:hidden" onClick={() => setMenuOpen(false)}>
          <div className="flex flex-col gap-1 pt-4">
            ...
            <Link href="/" className="text-[16px] text-[#1D1D1F] font-medium py-3 border-b border-[#F5F5F7] flex items-center gap-2"><Globe size={16} /> Deutsch</Link>

            <Link href="/impressum" className="text-[14px] text-[#86868B] py-2">Legal Notice</Link>
            <Link href="/datenschutz" className="text-[14px] text-[#86868B] py-2">Privacy Policy</Link>
          </div>
        </div>
      )}
    </>
      ...
      <div className="max-w-5xl mx-auto">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 sm:gap-10 mb-10 sm:mb-12">
          <div className="col-span-2 sm:col-span-1">
            ...
            <p className="text-[12px] text-[#86868B] leading-relaxed">
              The operating system for your AI team. Open Source. Swiss Made.
            </p>
          </div>
          {[
            { title: "Product", links: [
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-8 border-t border-[#E5E5EA]">
          ...
          <div className="flex items-center gap-5 text-[11px] text-[#C7C7CC]">
            <Link href="/" className="hover:text-[#6E6E73] transition-colors flex items-center gap-1"><Globe size={11} /> Deutsch</Link>
...
export default function EnglishShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-white text-[#1D1D1F]" lang="en" style={{ fontFamily: "var(--font-outfit, 'Outfit'), -apple-system, sans-serif" }}>
      <style>{`
